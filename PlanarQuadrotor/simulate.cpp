...
Eigen::MatrixXf LQR(PlanarQuadrotor &quadrotor, float dt) {
    ...
    Q.diagonal() << 2e-3, 2e-3, 4e2, 8e-3, 4.5e-2, 0.25 / 2 / M_PI;
    R.row(0) << 3e1, 7;
    R.row(1) << 7, 3e1;
    ...
    */
    ...
    if (init(gWindow, gRenderer, SCREEN_WIDTH, SCREEN_HEIGHT) >= 0)
    {
        ...
        while (!quit)
        {
                ...
                switch(e.type){
                    ...
                    case SDL_KEYDOWN:
                        if(e.key.keysym.sym == 112){
                            auto yw = plot(x_history, y_history);
                            show();
                            auto thetaw = plot(time, theta_history);
                            show();
                        }
                    break;
                }
            ...
            quadrotor_visualizer.render(gRenderer, t);
